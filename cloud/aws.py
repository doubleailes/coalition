...
def startInstance(name, config):
    ...
    if config.get("worker", "spot"):
        cmd = [
            ...
            "--spot-price",
            config.get("spot", "spotprice"),
            "--instance-count",
            config.get("spot", "instancecount"),
            "--type",
            config.get("spot", "type"),
            "--launch-specification",
            _getLaunchSpecification(name, config),
        ]
    else:
        cmd = [
            "aws",
            ...
            "run-instances",
            "--key-name",
            config.get("authentication", "keyname"),
            "--image-id",
            config.get("worker", "imageid"),
            "--instance-type",
            config.get("worker", "instancetype"),
            "--subnet-id",
            config.get("worker", "subnetid"),
            "--security-group-ids",
            config.get("worker", "securitygroupid"),
            "--iam-instance-profile",
            "Arn=%s" % config.get("worker", "iaminstanceprofile"),
            "--user-data",
            _getUserData(name, config),
        ]
...
def _getUserData(name, config):
    ...
    with open("cloud/aws_worker_cloud_init.template", "r") as f:
        ...
        values = {
            ...
            "secret_access_key": config.get("authentication", "secretaccesskey"),
            "bucket_name": config.get("storage", "name"),
            "mount_point": config.get("storage", "mountpoint"),
            "guerilla_render_filename": config.get("storage", "guerillarenderfilename"),
            "coalition_filename": config.get("storage", "coalitionfilename"),
