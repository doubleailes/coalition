...
import socket, time, subprocess, _thread, getopt, sys, os, base64, signal, string, re, platform, configparser, http.client, urllib.request, urllib.parse, urllib.error, datetime, threading
import random
from sys import modules
from os.path import splitext, abspath

from . import host_cpu, host_mem
...
if sys.platform == "win32":
    ...
    try:
        ...
        coalitionDir, type = winreg.QueryValueEx(hKey, "Installdir")
    except OSError:
        coalitionDir = "."
else:
    coalitionDir = "."
...
def workerRun(worker, func, retry):
    ...
    while gogogo:
        serverConn = None
        try:
            serverConn = http.client.HTTPConnection(re.sub("^http://", "", serverUrl))
...
class Worker:
    ...
    def mainLoop(self):
        ...
        if jobId != -1:
            ...
            _thread.start_new_thread(self.execProcess, (_cmd, _dir, user, _env))
            ...
            def endFunc(serverConn):
                params = urllib.parse.urlencode(
                    {"hostname": self.Name, "jobId": jobId, "errorCode": self.ErrorCode}
                )
...
def main():
    ...
    if workers == 1:
        ...
        worker = Worker(name)
        _thread.start_new_thread(threadfunc, (worker,))
    else:
