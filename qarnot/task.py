...
class Task(object):
    ...
    def _to_json(self):
        ...
        const_list = [
            {"key": key, "value": value} for key, value in list(self.constants.items())
        ]
        constr_list = [
            {"key": key, "value": value}
            for key, value in list(self.constraints.items())
        ]
...
class Error(object):
    ...
    def __str__(self):
        if sys.version_info > (3, 0):
            return ", ".join(
                "{0}={1}".format(key, val) for (key, val) in list(self.__dict__.items())
            )
        else:
            return ", ".join(
