...
def test_server_python_api():
    ...
    suite = unittest.TestSuite(list(map(ServerPythonApiTestCase, tests)))
    unittest.TextTestRunner(verbosity=VERBOSITY).run(suite)


def test_server_xmlrpc():
    tests = ["test_setJobDependencies"]
    suite = unittest.TestSuite(list(map(ServerXmlrpcTestCase, tests)))
    unittest.TextTestRunner(verbosity=VERBOSITY).run(suite)
...
class ServerPythonApiTestCase(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.depJobID = self.conn.newJob(
            command="echo dependencies", title="jobDependencies", state="PAUSED"
        )
        ...
        self.firstSleepJobId = self.conn.newJob(
            command="sleep 2",
            title="First Job",
            state="WAITING",
            affinity="linux",
        ...
        self.secondSleepJobId = self.conn.newJob(
            command="sleep 2",
            title="Second Job",
            state="WAITING",
            affinity="linux",
        ...
        self.windowsProjectJobId = self.conn.newJob(
            command="sleep 1",
            title="windows project",
            state="WAITING",
            affinity="windows project",
            priority=127,
        )
        self.winJobId = self.conn.newJob(
            command="sleep 1",
            title="Win",
            state="WAITING",
            affinity="win",
        ...
        self.dosJobId = self.conn.newJob(
            command="sleep 1",
            title="Dos",
            state="WAITING",
            affinity="dos",
        ...
        self.basicJobId = self.conn.newJob(
            command="sleep 1", title="Basic", state="WAITING", priority=300
        )
    ...
    def test_priorities(self):
        firstSleepJob = self.conn.getJob(self.firstSleepJobId)
        secondSleepJob = self.conn.getJob(self.secondSleepJobId)
        while not (secondSleepJob.state == "FINISHED") & (
            firstSleepJob.state == "FINISHED"
        ):
            time.sleep(1)
            firstSleepJob = self.conn.getJob(self.firstSleepJobId)
            secondSleepJob = self.conn.getJob(self.secondSleepJobId)
    ...
    def test_affinities_first(self):
        ...
        while (
            not (windowsProjectJob.state == "FINISHED")
            & (winJob.state == "FINISHED")
            & (dosJob.state == "FINISHED")
            & (basicJob.state == "FINISHED")
        ):
            windowsProjectJob = self.conn.getJob(self.windowsProjectJobId)
            winJob = self.conn.getJob(self.winJobId)
            dosJob = self.conn.getJob(self.dosJobId)
        ...
        self.assertTrue(
            (dosJob.start_time < basicJob.start_time)
            | (dosJob.worker != basicJob.worker)
        )
    ...
    def test_setJobDependencies(self):
        ...
        self.assertTrue(any([v in self.childrenID for v in depsID]))
...
class ServerXmlrpcTestCase(unittest.TestCase):
    ...
    def test_setJobDependencies(self):
        ...
        job1 = self.conn.newJob(
            0, "Test-1", "ls /", ".", "", "WAITING", False, 100, 15, "", "", "", ""
        )["id"]
        job2 = self.conn.newJob(
            0, "Test-2", "ls /", ".", "", "WAITING", False, 100, 15, "", "", "", ""
        )["id"]
    ...
    def test_worker2_pick_job(self):
        job3 = self.newJob(
            0, "Test-1", "ls /", ".", "", "PAUSED", False, 100, 15, "", "", "", ""
        )["id"]
        pick2 = self.pickJob("worker2", 1, 1, 1, "127.0.0.1")
        assertEqual(pick2[0], -1)
    ...
    def test_worker1_pick_job(self):
        job4 = self.newJob(
            0, "Test-1", "ls /", ".", "", "WAITING", False, 100, 10, "", "", "", ""
        )["id"]
        job5 = self.newJob(
            0, "Test-2", "ls /", ".", "", "WAITING", False, 100, 12, "", "", "", ""
        )["id"]
        pick1 = self.pickJob("worker1", 1, 1, 1, "127.0.0.1")
    ...
    def test_create_job7(self):
        job7 = self.newJob(
            0, "Parent-1", "", ".", "", "WAITING", False, 100, 10, "", "", "", ""
        )["id"]
        pick1 = self.pickJob("worker1", 1, 1, 1, "127.0.0.1")
        assertEqual(pick1[0], -1)
    ...
    def test_create_job8(self):
        job8 = self.newJob(
            job7, "Child-1", "ls /", ".", "", "WAITING", False, 100, 10, "", "", "", ""
        )["id"]
        pick1 = self.pickJob("worker1", 1, 1, 1, "127.0.0.1")
    ...
    def test_worker1_pick_job12(self):
        job9 = self.newJob(
            0, "Parent-2", "", ".", "", "WAITING", False, 100, 10, "", "", "", ""
        )["id"]
        job10 = self.newJob(
            0, "Parent-3", "", ".", "", "WAITING", False, 100, 11, "", "", "", ""
        )["id"]
        job11 = self.newJob(
            job9, "Child-2", "ls /", ".", "", "WAITING", False, 100, 10, "", "", "", ""
        )["id"]
        job12 = self.newJob(
            job10, "Child-3", "ls /", ".", "", "WAITING", False, 100, 8, "", "", "", ""
        )["id"]
        pick1 = self.pickJob("worker1", 1, 1, 1, "127.0.0.1")
    ...
    def test_intricate_dependencies(self):
        ...
        job20 = self.newJob(
            0, "job20", "sleep 2", ".", "", "PAUSED", False, 0, 100, "", "", "", ""
        )["id"]
        job21 = self.newJob(
            0, "job21", "", ".", "", "WAITING", False, 0, 100, "", "", "", ""
        )["id"]
        self.setJobDependencies(job21, [job20])
        job22 = self.newJob(
            job21, "job22", "sleep 2", ".", "", "WAITING", False, 0, 100, "", "", "", ""
        )["id"]
        job23 = self.newJob(
            0, "job23", "sleep 2", ".", "", "WAITING", False, 0, 150, "", "", "", ""
        )["id"]
        self.setJobDependencies(job23, [job21])
        job24 = self.newJob(
            0, "job24", "sleep 2", ".", "", "WAITING", False, 0, 200, "", "", "", ""
        )["id"]
